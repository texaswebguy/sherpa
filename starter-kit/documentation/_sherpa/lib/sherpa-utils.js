...
Sherpa.formatCurrency = function(number_data){
	...
	try {
		format = viewModel.settings.currency['format_'+format];
		// TODO what if viewModel.settings.currency is not configured correctly?
		amount = amount * viewModel.settings.currency.conversion_rate;
		amount = _.str.numberFormat(amount, format.decimals, format.decimal_separator, format.thousands_separator);
		position = viewModel.settings.currency.symbol_position;
		if (position == "left") {
			return viewModel.settings.currency.symbol+amount;
		} else {
			return amount+viewModel.settings.currency.symbol;
		}
	} catch (err) {
...
Sherpa.dateFormat = function(date) {
		...
		if(_.isObject(date)){
			try {
				...
				date = new Date();
				date_format = viewModel.settings.default_date_format;
			}
		} else {
			date_format = viewModel.settings.default_date_format;
		}
	...
	Sherpa.request({
		...
		success: function(responseHTML, status){
			...
			if(!_.isUndefined(Sherpa.componentJS[componentJS])) {
				...
				Sherpa.QA.check(responseHTML,SHERPA.COMPONENTS_PATH+component_name+"/"+filename);
			} else {
		...
		Sherpa.request({
			...
			success: function(responseHTML, status){
				...
				if(!_.isUndefined(Sherpa.includesJS[includeID])) {
					...
					//$(responseHTML).find('#'+componentJS).remove();
					$(element).html(responseHTML);
					Sherpa.QA.check(responseHTML,filename);
				} else {
					//set it right away because ajax takes its time
...
Sherpa.lorem = function(options){
	...
	if(_.isNumber(options) || _.isUndefined(options) ) {
		...
		if(_.isUndefined(options)) {
			options = viewModel.default_values.lorem_ispum_default;
		}
		return _.str.capitalize(_.str.prune((_.shuffle(viewModel.default_values.lorem_ispum).join(" ")).replace(",","").replace(regex,""),options,"."));
	} else {
		...
		if(!options.paragraphs) {
			if(_.isUndefined(options.length) || !_.isNumber(options.length) ) {
				options.length = viewModel.default_values.lorem_ispum_default;
			}
			return _.str.capitalize(options.startLorem+_.str.prune((_.shuffle(viewModel.default_values.lorem_ispum).join(" ")).replace(",","").replace(regex,""),options.length,"."));
		} else {
			if(_.isUndefined(options.length) || !_.isNumber(options.length) ) {
				options.length = viewModel.default_values.lorem_ispum_default;
			}
			...
			return (function(){
				...
				_.each(_.range(options.paragraphs), function(para) {
					...
					temp_html += '<p>'+options.startLorem+_.str.capitalize(_.str.prune((_.shuffle(viewModel.default_values.lorem_ispum).join(" ")).replace(",","").replace(regex,""),options.length,"."))+'</p>\n';
				});
				return temp_html;				
			})();
...
//sets up an android and iOS boolean
viewModel.IsiOsDevice = ko.observable(!_.isEmpty((window.navigator.userAgent.toLowerCase()).match("mac os")))
viewModel.IsAndroidDevice = ko.observable(!_.isEmpty((window.navigator.userAgent.toLowerCase()).match("android")));
viewModel.IsiWinRTDevice = ko.observable(!_.isEmpty((window.navigator.userAgent.toLowerCase()).match(/windows.*arm/)))
Sherpa.feature("android", viewModel.IsAndroidDevice());
Sherpa.feature("iOS", viewModel.IsiOsDevice());
Sherpa.feature("WinRT", viewModel.IsiWinRTDevice());


// Project specific information
Sherpa.ready("localJSApp", function() {
	window.setTimeout(function(){
		if(viewModel.default_values){
			Sherpa.request({
				resourceId: "project_info", 
				data: {
				  "filename" : viewModel.default_values.project_info_filename
				},
			...
			console.error("No project info file name found: viewModel.default_values.project_info_filename");
		} 
	},300);
});
Sherpa.counter("Sherpa Utils");
